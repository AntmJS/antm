...
function getAllModule(
  ...
  // 要不要 把 请求全部内敛进来
  const execute = async () => {
    const verifyFetchParams = funcTypes.filter((e: any) => {
      ...
      // 有一个出问题  就退出
      if (
        importType.importNames.includes(reqName) &&
        importType.importNames.includes(resName)
      ) {
        return true
      }
      console.log(
    ...
    try {
      ...
      verifyFetchParams.forEach((element: any) => {
      ...
      const containInter = await Promise.all(
        noInterface.map(async (e) => {
          let interfaceName: any = e.funcName
...
* Rap 地址: ${config!.rapper!.rapUrl}/repository/editor?id=${
            ...
            }&mod=${moduleId}&itf=${e.interfaceId}
*/
$2`,
          )
          // interface 第一行的位置
    ...
    await Promise.all(
      ...
      containInterface.map(async (el) => {
        ...
        await updateFileContent(
          importType.importPath,
          allFileMap[importType.importPath],
        )
        return result
      }),
    )
...
async function getFileInterface(config: IOptions, spinner: ora.Ora) {
  ...
  spinner.succeed(
    chalk.green(`[rapperPath] 目录中共扫描到${allFile.length}个文件`),
  )
  ...
  const allFileMap = allFileContent.reduce((c: any, n) => {
    if (n) {
      c[n.path] = n.content
    }
    return c
  ...
  const updateInterfaces = effectiveFile.map((el: any) => {
    return el.execute
  })
