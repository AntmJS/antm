 ...
 */
import type { Intf } from '../../types/index.d'
 ...
 */
export function findDeleteFiles(
  interfaces: Array<Intf>,
  excludePaths: string[],
) {
  ...
  fileList = fileList.filter((file) => {
    ...
    return !excludePaths.find(
      (exclude) => file.indexOf(resolve(process.cwd(), exclude)) > -1,
    )
  })
