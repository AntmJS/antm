import type { IModules, IUrlMapper, RAPPER_TYPE } from '../types/index.d'
import url from 'url'
...
import { getModules, creatHeadHelpStr } from './core/tools'
import { findDeleteFiles, findChangeFiles } from './core/scanFile'

const enum TRAILING_COMMA {
  NONE = 'none',
  ALL = 'all',
  ES5 = 'es5',
}
...
async function sanDeleteInterfaces(
  ...
  if (getOldProjectId(rapperPath) === String(projectId)) {
    ...
    if (scanResult.length && scanResult.length < 5) {
      spinner.warn(
        chalk.yellow('rapper: 如下文件使用了已被 Rap 删除或修改的接口'),
      )
      scanResult.forEach(({ key, filePath, start, line }) => {
        console.log(
          chalk.yellow(
            `    接口: ${key}, 所在文件: ${filePath}:${line}:${start}`,
          ),
        )
      })
...
export default async function rapper(config: IOptions) {
  ...
  const spinner = ora.default(chalk.grey('rapper: 开始检查版本'))
  spinner.start()
  /** 检查版本，给出升级提示 */
  // try {
  ...
  try {
    ...
    return new Promise(() =>
      spinner.fail(chalk.red(`rapper: 获取接口信息失败，${e}`)),
    )
  }
  //--
  ...
  try {
    ...
    await Promise.all(
      modules.map(async (m) => {
        ...
        const { tsInterfaceStr, tsCodeStr, tsInterfaceNames } =
          await createBaseRequestStr(
            interfaces,
            {
              rapUrl: rapUrl!,
              resSelector,
    ...
    outputFiles = outputFiles.map((item: any) => ({
      ...item,
  ...
  return Promise.all(
    outputFiles.map(({ path, content }: any) => writeFile(path, content)),
  )
    .then(() => {
      spinner.succeed(
        chalk.green(
