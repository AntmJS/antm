...
import CalendarDateList from '../ui/date-list/index'
import CalendarDayList from '../ui/day-list/index'

const ANIMTE_DURATION = 300

const defaultProps: Partial<CalendarBodyProps> = {
  marks: [],
  selectedDate: {
    end: Date.now(),
    start: Date.now(),
...
export default class CalendarBody extends Component<
  CalendarBodyProps,
  Readonly<CalendarBodyState>
> {
  static defaultProps: Partial<CalendarBodyProps> = defaultProps

  public constructor(props: CalendarBodyProps) {
    super(props)
    const {
      validDates,
      marks,
  ...
  public componentDidMount(): void {
    delayQuerySelector('.antmui-calendar-slider__main').then((res) => {
      this.maxWidth = res[0].width
    })
  }

  public UNSAFE_componentWillReceiveProps(nextProps: CalendarBodyProps): void {
    const {
      validDates,
      marks,
      format,
  ...
  private getGroups = (
    ...
    const arr: CalendarBodyListGroup = []
  ...
  private handleTouchStart = (e: ITouchEvent): void => {
    ...
    this.startX = e && e.touches && e.touches[0]!.clientX
  }
  ...
  private handleSwipeTouchStart = (
    ...
    const { clientY = 0, clientX = 0 } = (e && e.changedTouches[0]) ?? {}
      ...
      return (
        <View
          className={classnames(
            'main',
            'antmui-calendar-slider__main',
            `antmui-calendar-slider__main--${process.env.TARO_ENV}`,
          )}
        >
          <CalendarDayList />
        ...
        >
          ...
          >
            <View className="body__slider body__slider--pre">
              <CalendarDateList list={listGroup[0]!.list} />
            </View>
            <View className="body__slider body__slider--now">
              <CalendarDateList
                list={listGroup[1]!.list}
            ...
            <View className="body__slider body__slider--next">
              <CalendarDateList list={listGroup[2]!.list} />
            </View>
          </View>
        </View>
      )
