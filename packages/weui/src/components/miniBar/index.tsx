...
import { MiniBarProps } from '../../../types/miniBar'
        ...
        {backButton && (
          <div className="weui-minibar_left_back" onClick={handleGoBack}>
            {buttonColor === 'black' ? (
              ...
              <img
                className="weui-minibar_left_back_icon"
                style={{ width: '32px', height: '32px' }}
                src={
...
export default function Index(props: MiniBarProps): JSX.Element {
  ...
  useEffect(function () {
    ...
    const getMenuOption = function () {
      if (
        ...
        try {
          menuData = minins.getMenuButtonBoundingClientRect()
          needCallMenu = false
        } catch (error) {
          needCallMenu = false
        }
  ...
  const cls = `weui-minibar ${fixed ? 'weui-minibar_fixed' : ''} ${className}`
