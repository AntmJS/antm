...
module.exports = async function emailWarning(props) {
  if (!props.emailSender) {
    ...
    log.warining('set in cli like: antm-warning email --email-sender abc@163.com')
    log.warining('set in antm.config.js like: { warning: { emailSender: abc@163.com } }')
  ...
  if (!props.emailReceivers) {
    ...
    log.warining('set in cli like: antm-warning email --email-receivers abc@163.com')
    log.warining('set in antm.config.js like: { warning: { emailReceivers: abc@163.com } }')
  ...
  if (!props.emailSenderPass) {
    ...
    log.warining('set in cli like: antm-warning email --email-sender-pass hgfdsa5rew')
    log.warining('set in antm.config.js like: { warning: { emailSenderpass: hgfdsa5rew } }')

    process.exit(1)
  }
  if (!props.monitorFiles) {
    log.fail(`** please set monitor files **`)
    log.warining('set in cli like: antm-warning email --monitor-files package.json,README.md')
...
${res[key].replaceAll(/\n/, '<br />')}`
  ...
  SendEmail({
    ...
    log.success('send monitor files diff results to chart group success, reponse: ' + res.toString())
  })
}
...
function SendEmail({
  ...
  emailReceivers = Array.isArray(emailReceivers) ? emailReceivers.join(',') : emailReceivers
